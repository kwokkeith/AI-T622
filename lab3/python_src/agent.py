...
class VacuumCleanerAgent(Agent):
    ...
    def start(self, percepts):
        ...
        print("number of node expansions: %d" %
              self.search_algorithm.get_nb_node_expansions())
        print("node expansions per second: %.1f" % (
            self.search_algorithm.get_nb_node_expansions()/(end_time-start_time)))
        print("maximal frontier size: %d nodes" %
              self.search_algorithm.get_max_frontier_size())

        if not self.plan:
            raise Exception("no plan found")

        print("plan length: %d" % len(self.plan))
        print("effective branching factor: %.2f" % (math.log(
            self.search_algorithm.get_nb_node_expansions()) / math.log(len(self.plan))))
